...
**The log.** One append-only JSON-lines file per clinic, one record per
emission:

```json
{"array_count":1,"byte_count":57,"example_count":40,"kind":"fit",
 "metric_keys":["train_loss"],"payload_sha256":"6f0c…","round":1,
 "clinic_id":"north-general","status":"emitted",
...
against the clinic's own record of what it sent.
...
- honesty in the write-up: *"no raw data left the clinic"* is true and provable
...
These are worth answering with two real clinics before Stage 4, not after.
...
3. **Should evaluation be federated too?** A held-out set at each clinic gives
   honest per-clinic numbers, but multiplies the egress surface. The current
