...
`SiteTrainer`, `SiteManifest`, `Aperture`, in-process FedAvg, structure
...
**Exit criterion:** federated mean over three non-IID sites matches the pooled
...
| Module | Contents |
| --- | --- |
| `pymedphys._federated.protocol` | `SiteTrainer`, `SiteManifest`, `FitResult`, `EvalResult` |
...
Train the VAE on one site's cached data, single process, no federation. This
...
Split one physical dataset into N virtual sites. Split by *treating machine*,
...
baseline, and a demonstration that a deliberately mis-mapped site is rejected
before round 1.

## Stage 4 — two processes, then two hosts, then two sites
...
belong to neither contributing site. That is the claim worth making: the model
